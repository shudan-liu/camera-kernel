...
def gen_ais_headers(verbose, gen_dir, headers_install, unifdef, ais_include_uapi):
    error_count = 0
    for h in ais_include_uapi:
        ais_uapi_include_prefix = os.path.join(h.split('/include/uapi/')[0],
                                                 'include',
                                                 'uapi',
                                                 'ais') + os.sep
        ...
        if not run_headers_install(
                ...
                ais_uapi_include_prefix, h): error_count += 1
    return error_count
...
def main():
    ...
    parser.add_argument(
            '--ais_include_uapi', required=True, nargs='*',
    ...
    if args.verbose:
        ...
        print('ais_include_uapi [%s]' % args.ais_include_uapi)
        print('headers_install [%s]' % args.headers_install)
        print('unifdef [%s]' % args.unifdef)

    return gen_ais_headers(args.verbose, args.gen_dir,
            args.headers_install, args.unifdef, args.ais_include_uapi)
