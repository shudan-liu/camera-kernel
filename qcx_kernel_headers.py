...
def gen_qcx_headers(verbose, gen_dir, headers_install, unifdef, qcx_include_uapi):
    error_count = 0
    for h in qcx_include_uapi:
        qcx_uapi_include_prefix = os.path.join(h.split('/include/uapi/')[0],
                                                 'include',
                                                 'uapi',
                                                 'camera') + os.sep
        ...
        if not run_headers_install(
                ...
                qcx_uapi_include_prefix, h): error_count += 1
    return error_count
...
def main():
    ...
    parser.add_argument(
            '--qcx_include_uapi', required=True, nargs='*',
    ...
    if args.verbose:
        ...
        print('qcx_include_uapi [%s]' % args.qcx_include_uapi)
        print('headers_install [%s]' % args.headers_install)
        print('unifdef [%s]' % args.unifdef)

    return gen_qcx_headers(args.verbose, args.gen_dir,
            args.headers_install, args.unifdef, args.qcx_include_uapi)
